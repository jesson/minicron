...
(function() {
  Minicron.Schedule = DS.Model.extend({
    ...
    // so that it can be kept up to date on the frontend fs.writeFile(path, data, encoding_, callback);
    // changes are made
    formatted: function() {
      if (this.get('special') === null) {
        var formatted =  (this.get('minute') != null ? this.get('minute') : '*') + ' ';
            formatted += (this.get('hour') != null ? this.get('hour') : '*') + ' ';
            formatted += (this.get('day_of_the_month') != null ? this.get('day_of_the_month') : '*') + ' ';
            formatted += (this.get('month') != null ? this.get('month') : '*') + ' ';
            formatted += this.get('day_of_the_week') != null ? this.get('day_of_the_week') : '*';
