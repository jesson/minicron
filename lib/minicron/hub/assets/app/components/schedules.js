...
(function() {
  ...
  function parseSchedule(schedule) {
    ...
    if (schedule.substr(0, 1) === '@') {
      ...
      $panel = Ember.$('#schedule-editor').find('.panel').eq(5);

      // Show the panel
      $panel.find('> a').trigger('click');

      // Select the correct radio button
      $panel.find('input[type="radio"]').each(function(k ,v) {
        var $this = $(this);

        if ($this.data('value') === schedule) {
          $this[0].checked = true;
    ...
    // Otherwise handle the other types of schedule
    } else {
      ...
      for (var i = 0; i < expression.length; i++) {
        ...
        // Select the panel for this expression
        $panel = Ember.$('#schedule-editor').find('.panel').eq(i);
  ...
  function handleUpdate(self, $this) {
    ...
    if (typeof $this.attr('href') !== 'undefined') {
      ...
      // If it's a checkbox/radio set the id to it's parents parent
    ...
    // Is it a radio button, i.e special
    else if (id === 'special') {
      key = id;

      // Loop every radio button
      Ember.$('#' + id).find('input[type="radio"]').each(function(k, v) {
        if ($this[0] == v) {
      ...
      // If no value has been set i.e no radio buttons are ticked, default to hourly
      if (value.length === 0) {
        var $hourly = Ember.$($this.context).find('.btn-group input[type="radio"]').first();
        $hourly[0].checked = true;
        $hourly.closest('label').addClass('active');
        value = '@hourly';
      }
    }
    ...
    // This is a little bit nasty but if the schedule wasn't a special one
    // we need to make sure that the schedule property on the model is null
    if (id !== 'special') {
      schedule.set('special', null);
    }
  ...
  Minicron.ScheduleEditorComponent = Ember.Component.extend({
    didInsertElement: function() {
      ...
      // Handle when one of the accordion panels is shown
      Ember.$('#schedule-editor').on('show.bs.collapse', function (e) {
        // Pass a reference to the active tab so that it works in the same way as below
        handleUpdate(self, Ember.$(e.target).find('.nav-tabs .active > a'));
      });
    ...
    actions: {
      ...
      cancel: function(schedule) {
        this.sendAction('cancel', this.get('schedule'));
      }
    }
  });
})();
